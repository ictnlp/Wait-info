...
class SimSequenceGenerator(nn.Module):
    def __init__(
        self,
        models,
        tgt_dict,
    ...
    def _generate(
        ...
        assert (
            net_input["src_tokens"].size(0) == 1
        ), "batch-size in simultaneous decoding must be 1"
        ...
        src_info = encoder_outs[0][1]
        encoder_outs = [encoder_outs[0][0]]
        finalized_src_info = src_info
        # pdb.set_trace()
        ...
        # src_norm=torch.norm(encoder_outs[0][0],dim=-1).transpose(0,1)*(1-encoder_outs[0][1].int())
        ...
        while step < max_len + 1:
            ...
            if reorder_state is not None:
                ...
                src_info = src_info.index_select(0, reorder_state)
            ...
            partial_net_input, cur_net_input = self.bulid_partial_net_input(
                cur_net_input, cur_read_len, reorder_state
            )
            ...
            lprobs, avg_attn_scores, tgt_info = self.model.forward_decoder(
                tokens[:, : step + 1],
                encoder_outs,
                incremental_states,
                self.temperature,
            ...
            if (
                src_info.sum() < tgt_info.sum() + test_waitk_lagging - 1
                and cur_read_len < net_input["src_lengths"]
            ):
                # ------------READ------------#
                cur_read_len += 1
                continue
            ...
            # ------------WRITE------------#
            read = (avg_attn_scores > 0).sum(dim=-1, keepdim=False)[:, :, -1]
            ...
            if eos_bbsz_idx.numel() > 0:
                ...
                for idx in finalized_sents:
                    ...
                    finalized_tgt_info[ids[idx].item()] = tgt_info[idx]
    ...
    def forward_decoder(
        ...
        for i, model in enumerate(self.models):
            ...
            if self.models_size == 1:
                return probs, attn, decoder_out[1]["tgt_info"]
