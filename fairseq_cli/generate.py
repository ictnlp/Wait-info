...
def _main(cfg: DictConfig, output_file):
    ...
    for sample in progress:
        ...
        if "net_input" not in sample:
            continue
        # if 128 not in sample["id"].tolist(): continue
        ...
        gen_timer.start()
        hypos, d, srcs, src_info, tgt_info = task.inference_step(
            generator,
            models,
            sample,
        ...
        for i, sample_id in enumerate(sample["id"].tolist()):
            ...
            if align_dict is not None:
                ...
                if src_dict is not None:
                    ...
                    bpe_src_str = src_dict.string(src_tokens)
                else:
                    src_str = ""
                if has_target:
            ...
            for j, hypo in enumerate(hypos[i][: cfg.generation.nbest]):
                ...
                bpe_tgt_str = tgt_dict.string(hypo["tokens"].int().cpu())
                ...
                if not cfg.common_eval.quiet:
                    ...
                    cur_src_info = np.around(src_info[i].squeeze(-1).cpu().numpy(), 2)
                    cur_src_info = cur_src_info[cur_src_info != 0]
                    cur_src_info_str = [str(_) for _ in cur_src_info]
                    print(
                        "SI-{}\t{}\t{}".format(
                            sample_id,
                            np.around(np.mean(cur_src_info), 2),
                            ",".join(cur_src_info_str),
                        ),
                        file=output_file,
                    )
                    cur_tgt_info = np.around(
                        tgt_info[sample_id].squeeze(-1).cpu().numpy(), 2
                    )
                    cur_tgt_info = cur_tgt_info[cur_tgt_info != 0]
                    cur_tgt_info_str = [str(_) for _ in cur_tgt_info]
                    print(
                        "TI-{}\t{}\t{}".format(
                            sample_id,
                            np.around(np.mean(cur_tgt_info), 2),
                            ",".join(cur_tgt_info_str),
                        ),
                        file=output_file,
                    )
                    print(
                        "RW-{}\t{}\t{}".format(
                            sample_id, np.around(RW2AL(rw01), 2), rw01
                        ),
                        file=output_file,
                    )
